...
abstract class Registry<in R, out T>(val namer: Namer<R>, val default: (R) -> T) {
    ...
    fun <U : @UnsafeVariance T> register(name: Name, builder: (@UnsafeVariance R) -> U) {
        simpleNames.put(name, builder)
            ?: throw IllegalStateException("cannot override registered elements")
    }
...
object AttributeRegistry :
    Registry<RealAttribute, Attribute<*>>(
        RealAttributeNamer,
        { name -> Attribute<String>(name.name) })
