...
actual suspend fun AvailableLocalizations.fetchLanguage(
    ...
    val url: String = tag.makeLessSpecific { languages[it] }
        ?: languages[defaultLanguage]!!

    val response = fetch("/_" + resolver.getTargetUrl(url))
    if (!response.ok) {
        logger.warn { "Status code ${response.status}: ${response.statusText} while fetching resource for tag $tag" }

        if (tag == defaultLanguage) {
            throw IllegalStateException("Default language could not be fetched")
        } else {
